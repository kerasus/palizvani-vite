      ...
      <template v-else-if="inputData.col.name === 'actions'">
        <div class="action-column-entity-index">
          <q-btn size="md"
                 color="primary"
                 label="جزییات"
                 :to="{name: 'Admin.Event.Project.Show', params: {event_id: $route.params.id, project_id: inputData.props.row.id}}"
                 class="q-mr-md" />
      ...
      <template v-else-if="inputData.col.name === 'members'">
        <div class="action-column-entity-index">
          <q-btn size="md"
                 ...
                 outline
                 label="بررسی پروژه"
                 :to="{name: 'Admin.Event.Project.Members', params: {event_id: $route.params.id, project_id: inputData.props.row.id}}"
                 class="q-mr-md" />
        </div>
      </template>
      <template v-else>
...
export default {
  ...
  methods: {
    addNewProject () {
      this.$router.push({ name: 'Admin.Event.Project.Create', params: { event_id: this.eventId } })
