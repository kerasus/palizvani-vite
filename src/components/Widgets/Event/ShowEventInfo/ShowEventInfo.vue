  ...
  <div class="event">
    ...
    <q-card v-if="!loading"
      ...
      <q-card-section class="info-box">
        ...
        <q-banner v-if="event.getUserRegisterInfoLabel()"
                  inline-actions
                  rounded
                  class="bg-blue-3 text-white text-center q-mb-md">
          {{ event.getUserRegisterInfoLabel() }}
        </q-banner>

        <div class="row">
          <div class="col-md-4 col-12 event-thumbnail">
            <q-img :src="event.thumbnail" />
          </div>
          <div class="col-md-4 col-12 event-top-attributes">
            <div class="title">
              رویداد
              {{event.title}}:
            </div>
            <div class="attributes">
              <div class="attribute-item">{{getAudienceGenderType(event.audience_gender_type)}}</div>
              <!--              <div class="attribute-item">{{getHoldingType(event.holding_type)}}</div>-->
            </div>
          </div>
          <div class="col-md-4 col-12 event-action-col">
            <q-btn v-if="event.is_enabled_adding"
                   color="primary"
            ...
            <q-btn v-if="event.is_enabled_enrolment"
                   color="primary"
                   class="btn-register q-ml-md"
                   @click="onEnrolment">
              <svg data-name="Add User"
            ...
            <q-btn v-if="!defaultOptions.profileMode && event.is_enabled_viewing"
                   color="primary"
                   class="btn-register q-ml-md"
                   :to="{name: 'UserPanel.Profile.EventInfo', params: {id: event.id}}">
              مشاهده رویداد
            </q-btn>
            <q-btn v-if="event.is_enabled_dropping"
                   outline
                   color="red"
                   class="btn-register q-ml-md"
                   :loading="dropLoading"
            ...
            <q-btn v-if="event.current_user_register_info?.status !== 'REGISTERED' && event.current_user_register_info?.status !== 'ENROLLED'"
                   outline
                   color="primary"
                   class="btn-register q-ml-md"
      ...
      <q-card-section class="info-box">
        <div class="row second-section">
          <div class="col-md-4 col-sm-6 col-12 attribute-column">
            ...
            <div class="title">
              {{ event.professor_info.firstname }}
              {{ event.professor_info.lastname }}
            </div>
          </div>
          <div class="col-md-4 col-sm-6 col-12 attribute-column">
            <div class="icon">
            ...
            <div class="title">
              <!--              {{ getTerm(event) }}-->
              {{getHoldingType(event.holding_type)}}
            </div>
          </div>
          <div class="col-md-4 col-sm-6 col-12 attribute-column">
            <div class="icon">
            ...
            <div class="title">
              {{ event.price ? event.price.toLocaleString('fa') : 0 }}
              ریال
            </div>
          </div>
        </div>
    ...
    <q-dialog v-model="rulesDialog"
      ...
      <q-card>
        ...
        <q-card-section style="max-height: 50vh"
                        class="scroll">
          <div v-html="event.rules" />
          <div v-if="event.unit_info && event.unit_info.codes"
               v-html="event.unit_info.codes" />
        </q-card-section>

        <q-separator />

        <q-card-actions class="rulesDialog-actions">
          <div v-if="(!event.is_enabled_enrolment || !event.is_enabled_adding) && acceptClassType !== 'rules'"
               class="rulesDialog-accept-chk">
            <q-checkbox v-model="rulesAccept"
                        label="قوانین رویداد آموزشی را خواندم و قصد ثبت نام در رویداد را دارم" />
          </div>
          <div class="rulesDialog-btns q-gutter-md">
            <q-btn v-if="acceptClassType === 'rules'"
                   ...
                   color="primary"
                   outline />
            <q-btn v-if="(!event.is_enabled_enrolment || !event.is_enabled_adding) && acceptClassType !== 'rules'"
                   v-close-popup
                   label="انصراف"
                   color="primary"
                   outline />
...
export default {
  ...
  methods: {
    ...
    reloadEvent () {
      this.getEvent()
        .then((event) => {
          this.prefetchServerDataPromiseThen(event)
        })
        .catch(() => {
    ...
    openCodeLink () {
      ...
      const fileName = (this.event.codes.match(/(?<=\/)[^/?#]+(?=[^/]*$)/gm) || [this.event.title])[0]
      this.fetchCodesLoading = true
      fetch(this.event.codes)
    ...
    getAudienceGenderType (type) {
      const target = (new Event()).audience_gender_typeEnums.find(item => item.value === type)
      if (target) {
        return target.label
      }
      return '-'
