<template>
  <div class="AdminContentTopicCategoryList"
       :style="localOptions.style">
    <div v-if="!parent"
         class="more-action">
      <q-btn label="دسته موضوعی جدید"
             color="primary"
             :to="{name: 'Admin.ContentTopicCategory.Create'}" />
    </div>
    <entity-index v-if="mounted"
      ...
      <template #entity-index-table-cell="{inputData}">
        ...
        <template v-else-if="inputData.col.name === 'action'">
          <q-btn color="primary"
                 :to="{name: 'Admin.ContentTopicCategory.Show', params: {id: inputData.props.row.id}}">
            مشاهده جزییات
          </q-btn>
        </template>
        <template v-else>
...
export default {
  name: 'AdminContentTopicCategoryList',
