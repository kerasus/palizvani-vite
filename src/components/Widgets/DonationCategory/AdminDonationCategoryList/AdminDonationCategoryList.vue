<template>
  <div class="AdminDonationCategoryList"
       :style="localOptions.style">
    <div v-if="!parent"
         class="more-action">
      <q-btn label="دسته جدید"
             color="primary"
             :to="{name: 'Admin.DonationCategory.Create'}" />
    </div>
    <entity-index v-if="mounted"
      ...
      <template #entity-index-table-cell="{inputData}">
        ...
        <template v-else-if="inputData.col.name === 'action'">
          <q-btn color="primary"
                 :to="{name: 'Admin.DonationCategory.Show', params: {id: inputData.props.row.id}}">
            مشاهده جزییات
          </q-btn>
        </template>
        <template v-else>
...
export default {
  name: 'AdminDonationCategoryList',
  components: { EntityIndex },
  mixins: [mixinWidget],
  props: {
    title: {
      type: String,
      default: 'لیست دسته بندی های کمک های مالی'
    }
  },
  data: () => {
    return {
