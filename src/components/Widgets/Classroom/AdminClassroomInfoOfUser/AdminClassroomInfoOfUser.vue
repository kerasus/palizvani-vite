<template>
  ...
  <q-card class="q-mb-md q-pa-sm">
    <q-linear-progress v-show="ownerInfo.loading"
                       indeterminate />
    <div class="row items-center justify-center">
      <div class="col-md-1">
        <q-img :src="ownerInfo.picture" />
      </div>
      <div class="col-md-11">
        <div class="row items-center">
          <div class="col-md-4">
            {{ ownerInfo.gender === 'MALE' ? 'آقای' : 'خانم' }}
            {{ ownerInfo.fullname() }}
            <br>
            کد ملی:
            {{ ownerInfo.national_code }}
          </div>
          <div class="col-md-4">
            تلفن همراه جهت تماس:
          ...
          <div class="col-md-4">
            <q-btn color="primary"
                   :to="{ name: 'Admin.User.Show', params: { id: ownerId }}"
                   target="_blank">
              مشاهده پروفایل
            </q-btn>
          </div>
  ...
  <q-card>
    ...
    <q-tab-panels v-model="tab"
      ...
      <q-tab-panel name="classroomInfo">
        ...
        <show-classroom-info v-if="classroom.id"
...
import ShowClassroomInfo from './components/ShowClassroomInfo.vue'
...
export default {
  ...
  computed: {
    ...
    canShowOtherTabsOfClassroom () {
      return !this.classroom.user_register_info.loading && (this.classroom.user_register_info.status === 'REGISTERED' || this.classroom.user_register_info.status === 'ENROLLED')
    },
    selectedCategoryId () {
  ...
  methods: {
    getClassroom () {
      ...
      APIGateway.classroom.getClassroomAndCorrespondingRegistration(this.classroomId, this.ownerId)
        .then((classroom) => {
          this.classroom = new Classroom(classroom)
          if (!this.classroom.user_register_info.id) {
            this.getRegistrationInfo()
          }
          this.onloadnClassroom()
        })
        .finally(() => {
    ...
    getUserInfo () {
      this.ownerInfo.loading = true
      APIGateway.user.get({ data: { id: this.ownerId } })
        .then((user) => {
          this.ownerInfo = new User(user)
        })
        .finally(() => {
          this.ownerInfo.loading = false
        })
    },
    ...
    getRegistrationInfo () {
      ...
      this.classroom.user_register_info.loading = true
      APIGateway.registration.index({ classroom: this.classroomId })
        .then(({ list }) => {
          this.registrations = new RegistrationList(list)
          if (this.registrations.list.length > 0) {
            this.classroom.user_register_info = this.registrations.list[0]
          }
          this.registrations.loading = false
          this.classroom.user_register_info.loading = false
          this.onloadnClassroom()
        })
        .catch(() => {
          this.registrations.loading = false
