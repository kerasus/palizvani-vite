...
export default {
  ...
  data () {
    const userId = parseInt(this.$route.params.user_id)
    ...
    return {
      ...
      mounted: false,
      api: APIGateway.session.APIAdresses.sessionAndCurrentUserAttendanceSheetOfUser(sessionId, userId),
      sessionInfoInputs: [
      ...
      submitAttendanceStatusInputs: [
        ...
        { type: 'hidden', name: 'owner', responseKey: 'owner', value: userId }
    ...
    updateBreadcrumbs () {
      this.$store.commit('AppLayout/updateBreadcrumbs', {
        ...
        path: [
          ...
          {
            label: 'جزییات جلسه',
            to: { name: 'UserPanel.Profile.SessionInfo', params: { id: this.$route.params.session_id } }
          }
        ]
      })
    },
